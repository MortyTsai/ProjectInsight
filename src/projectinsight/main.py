...
def process_project(config_path: Path):
    ...
    analysis_type = config.get("analysis_type", "dependency")
    architecture_layers = config.get("architecture_layers", {})
    vis_config = config.get("visualization", {})
    dep_graph_config = vis_config.get("dependency_graph", {})
    flow_graph_config = vis_config.get("control_flow_graph", {})
    ...
    if analysis_type == "dependency":
        layout_engine = dep_graph_config.get("layout_engine", "dot")
        logging.info(f"使用佈局引擎 '{layout_engine}' 進行渲染...")
        output_path = output_dir / f"{config_path.stem}_dependency_{layout_engine}.png"

        all_module_details = {}
        for file_path in py_files:
            if file_path.name == "__init__.py":
                continue
            module_name = ".".join(file_path.relative_to(target_project_src.parent).with_suffix("").parts)
            all_module_details[module_name] = parsers.py_parser.analyze_dependencies(file_path)

        graph_data = builders.build_graph_data(all_module_details, root_package_name)
        renderers.render_graph(
            graph_data=graph_data, output_path=output_path, root_package=root_package_name,
            save_source_file=True, layer_info=architecture_layers, layout_engine=layout_engine
        )

    elif analysis_type == "control_flow":
        layout_engine = flow_graph_config.get("layout_engine", "dot")
        show_internal_calls = flow_graph_config.get("show_internal_calls", True)
        logging.info(f"使用佈局引擎 '{layout_engine}'... (內部呼叫: {'顯示' if show_internal_calls else '隱藏'})")
        output_path = output_dir / f"{config_path.stem}_control_flow_{layout_engine}.png"

        analysis_results = parsers.flow_parser.analyze_code(target_project_src, root_package_name, py_files)
        all_modules = set(analysis_results["module_details"].keys())

        graph_data = builders.build_flow_graph_data(
            call_graph=analysis_results["call_graph"],
            show_internal_calls=show_internal_calls,
            all_modules=all_modules
        )
        renderers.render_flow_graph(
            graph_data=graph_data, output_path=output_path, root_package=root_package_name,
            save_source_file=True, layer_info=architecture_layers, layout_engine=layout_engine
        )
    else:
