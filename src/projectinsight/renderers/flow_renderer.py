# src/projectinsight/renderers/flow_renderer.py
"""
封裝控制流圖的 Graphviz 渲染邏輯。
"""
# 1. 標準庫導入
import logging
import subprocess
...
def render_flow_graph(
    ...
    title = (
        f'<<FONT {font_face} POINT-SIZE="20">{root_package} 控制流圖 引擎: {layout_engine}</FONT><BR/>'
        f'<FONT {font_face} POINT-SIZE="12">箭頭 A -&gt; B 表示 A 呼叫 B</FONT>>'
    )

    dot.attr(fontname='Microsoft YaHei')
    if layout_engine == 'dot':
        dot.attr(rankdir='TB', splines='ortho', nodesep='0.5', ranksep='1.0',
                 label=title, charset='UTF-8', compound='true')
    else:
        dot.attr(splines='true', nodesep='0.2', label=title, charset='UTF-8',
                 compound='true', overlap='prism')
