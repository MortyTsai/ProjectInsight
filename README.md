...
-   **LLM 優先的上下文輸出 (LLM-First Context Output)**:
    -   **高效文字圖**: 專為 LLM 設計，將高階組件關係圖抽象為 Token 極度高效的「鄰接串列」格式，使 LLM 能以最少的 Token 消耗理解最核心的架構關係。
    -   **職責分離的產出**: 嚴格區分產出目標，為 LLM 生成純粹的 `.md` 上下文報告，為人類開發者生成用於視覺化除錯的 `.png` 圖檔和 `.log` 除錯日誌。

-   **健壯的解析與渲染引擎 (Robust Parsing & Rendering Engine)**:
    -   **專案結構自適應**: 無需手動設定，即可自動偵測 `src` 佈局、扁平佈局及包含多個頂層套件的複雜專案結構，並建立正確的解析上下文。
    -   **拓撲感知佈局**: 渲染引擎能夠識別圖的拓撲結構，對「連通分量」進行分組和打包佈局，確保在分析包含大量獨立模組的大型、低內聚專案時，依然能生成佈局合理、尺寸可控的視覺化圖表。
    -   **智慧化大型專案嚮導**: 內建互動式精靈，可在分析大型專案時，基於啟發式、圖論中心性和生態位分析，為使用者推薦高品質的分析入口點，並支援聚焦分析。
...
-   [ ] **第三階段 (長期)：深度語義分析與生態擴展**
    -   [ ] 設計並實作一個外掛系統，允許為特定框架或語言特性開發專用的解析器。